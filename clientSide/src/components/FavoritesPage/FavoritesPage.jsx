...
// import { LayoutGrid, List, Filter, MapPin, Trash2, Heart, ChevronDown, X, DollarSign, Calendar, Eye, Star, Share2, Bell, Truck, Wrench } from 'lucide-react';
...
//     { id: "excavators", name: "حفارات", icon: <Wrench size={14} /> },
//     { id: "bulldozers", name: "جرافات", icon: <Wrench size={14} /> },
//     { id: "cranes", name: "رافعات", icon: <Wrench size={14} /> },
//     { id: "trucks", name: "شاحنات", icon: <Truck size={14} /> },
//     { id: "generators", name: "مولدات", icon: <Wrench size={14} /> }
//   ];
...
//     switch (condition) {
//       case "جديدة": return "bg-green-100 text-green-800 border-green-200";
//       case "ممتازة": return "bg-blue-100 text-blue-800 border-blue-200";
//       case "جيدة": return "bg-amber-100 text-amber-800 border-amber-200";
//       case "مستعملة": return "bg-orange-100 text-orange-800 border-orange-200";
...
//       try {
//         const response = await axios.get("http://localhost:5000/api/users/favorites", { withCredentials: true });
...
//     return item.dailyRate >= priceRange[0] && item.dailyRate <= priceRange[1];
//   });
...
//       {children}
//       <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 -translate-y-1 px-2 py-1 bg-amber-500 text-white text-xs rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 whitespace-nowrap z-10 shadow-lg shadow-amber-500/20">
//         {text}
...
//                 <div className="ml-3 w-10 h-10 rounded-lg bg-amber-500 text-white flex items-center justify-center shadow-md">
//                   <Heart size={20} />
//                 </div>
//                 <span className="relative">
//                   المعدات المفضلة
//                   <span className="absolute -bottom-1 right-0 w-full h-1 bg-amber-500/30 rounded-full"></span>
//                 </span>
//               </h1>
...
//                       : 'bg-white text-gray-700 border-gray-200 hover:border-amber-500 hover:text-amber-500'
//                   }`}
//                 >
//                   <LayoutGrid size={18} />
...
//                       : 'bg-white text-gray-700 border-gray-200 hover:border-gray-800 hover:text-gray-800'
//                   }`}
//                 >
//                   <Filter size={18} />
...
//               {/* Price Range Filter */}
//               <div>
//                 <h3 className="text-lg font-medium text-gray-800 mb-3">نطاق السعر (دينار/يوم)</h3>
...
//               {/* Sort Options */}
//               <div>
//                 <h3 className="text-lg font-medium text-gray-800 mb-3">ترتيب حسب</h3>
...
//                   : 'bg-white text-gray-700 border border-gray-200 hover:border-amber-500 hover:text-amber-500'
//               }`}
//             >
...
//         {hasFavorites && sortedFavorites.length > 0 ? (
//           <div
//             className={`
//               ${viewMode === 'grid' 
...
//                   ${isLoaded ? 'opacity-100 translate-y-0' : ''}
//                   border border-gray-200 hover:border-amber-500
...
//                   />
//                   {/* Condition badge */}
//                   <div className={`absolute top-3 right-3 px-2 py-1 rounded-md text-xs font-medium ${getConditionColor(item.condition)}`}>
//                     {item.condition || "جيدة"}
//                   </div>
...
//                   {/* Title */}
//                   <h2 className="text-lg font-bold text-gray-800 mb-2 line-clamp-1 group-hover:text-amber-500 transition-colors">
//                     {item.title}
//                   </h2>

//                   {/* Manufacturer and model */}
//                   <div className="flex items-center text-sm mb-3 text-gray-600">
//                     <Wrench size={14} className="ml-1.5 text-amber-500" />
//                     <span className="mr-1">{item.manufacturer} {item.model} - {item.year}</span>
//                   </div>

//                   {/* Location */}
//                   <div className="flex items-center text-sm mb-3 text-gray-600">
//                     <MapPin size={14} className="ml-1.5 text-amber-500" />
//                     <span className="mr-1">{item.location || "عمان، الأردن"}</span>
//                   </div>
...
//                       <span className="text-gray-700 font-medium">{item.averageRating || "4.5"}</span>
//                     </div>
...
//             <div className="w-24 h-24 mx-auto mb-8 rounded-full bg-amber-100 flex items-center justify-center transform hover:rotate-6 transition-all duration-700 hover:scale-110 group">
//               <Heart size={36} className="text-amber-500 transition-all duration-700 group-hover:scale-110" />
//             </div>
...
//               قم بإضافة معدات إلى المفضلة لمتابعتها ومقارنتها لاحقًا. يمكنك النقر على أيقونة القلب في صفحة المعدات لإضافتها إلى المفضلة.
//             </p>
//             <Link to="/equipment" className="inline-block bg-amber-500 hover:bg-amber-600 text-white px-6 py-3 rounded-md transition-colors font-medium">
...
//                 معدات مقترحة لك
//                 <span className="absolute -bottom-1 right-0 w-full h-0.5 bg-amber-500/50 rounded-full"></span>
//               </h3>
//               <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
//                 {[1, 2, 3].map((item) => (
//                   <div key={item} className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 hover:border-amber-500 group">
...
//                         جديدة
//                       </div>
//                       <button className="absolute top-3 left-3 w-8 h-8 rounded-full bg-white/80 backdrop-blur-sm text-gray-400 flex items-center justify-center hover:bg-amber-500 hover:text-white transition-colors">
//                         <Heart size={16} />
//                       </button>
//                     </div>
//                     <div className="p-4">
//                       <h4 className="font-bold text-gray-800 mb-2 group-hover:text-amber-500 transition-colors">
...
//                         <div className="flex items-center">
//                           <Star size={16} className="ml-1 text-yellow-400 fill-yellow-400" />
//                           <span className="text-gray-700 font-medium">4.{7 + item}</span>
//                         </div>
//                         <div className="flex items-center text-amber-500 font-bold">
...
//             <h3 className="text-xl font-bold text-gray-800 mb-2">لا توجد نتائج تطابق الفلتر</h3>
//             <p className="text-gray-600 mb-4">يرجى تعديل معايير البحث أو إعادة ضبط الفلتر للعثور على المعدات</p>
//             <button
//               onClick={() => {
//                 setActiveCategory("all");
//                 setPriceRange([0, 1000]);
...
const FavoritesPage = () => {
  ...
  const categories = [
    { id: "all", name: "الكل", icon: <Wrench size={14} /> },
    { id: "آليات ثقيلة", name: "آليات ثقيلة", icon: <Construction size={14} /> },
  ...
  const getConditionColor = (condition) => {
    switch (condition) {
      case "جديدة": return "bg-green-100 text-green-800 border-green-200";
      case "ممتازة": return "bg-blue-100 text-blue-800 border-blue-200";
      case "جيدة جداً": return "bg-indigo-100 text-indigo-800 border-indigo-200";
      case "جيدة": return "bg-amber-100 text-amber-800 border-amber-200";
      case "مستعملة": return "bg-orange-100 text-orange-800 border-orange-200";
      case "كالجديدة": return "bg-teal-100 text-teal-800 border-teal-200";
      default: return "bg-gray-100 text-gray-800 border-gray-200";
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12" dir="rtl">
        ...
        <div className="bg-white shadow-md border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {/* City Filter */}
              <div>
                <h3 className="text-lg font-medium text-gray-800 mb-3">المدينة</h3>
            ...
            {(selectedCity || priceRange[0] > 0 || priceRange[1] < 5000) && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  {selectedCity && (
                    <div className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-sm flex items-center">
                      <span className="ml-1">المدينة: {selectedCity}</span>
                      <button onClick={() => setSelectedCity("")} className="text-amber-700 hover:text-amber-900">
                        <X size={14} />
                      </button>
                    </div>
                  )}
                  ...
                  {(priceRange[0] > 0 || priceRange[1] < 5000) && (
                    <div className="bg-amber-100 text-amber-700 px-3 py-1 rounded-full text-sm flex items-center">
                      <span className="ml-1">السعر: {priceRange[0]} - {priceRange[1]} دينار</span>
                      <button onClick={() => setPriceRange([0, 5000])} className="text-amber-700 hover:text-amber-900">
                        <X size={14} />
                      </button>
                    </div>
                  )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {/* No matches found */}
        {hasFavorites && sortedFavorites.length > 0 && currentItems.length === 0 && (
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            <Filter size={48} className="mx-auto mb-4 text-gray-300" />
            <h3 className="text-xl font-bold text-gray-800 mb-2">لا توجد نتائج تطابق الفلتر</h3>
            <p className="text-gray-600 mb-4">يرجى تعديل معايير البحث أو إعادة ضبط الفلتر للعثور على المعدات</p>
