...
// // تبديل المعدات في المفضلة (إضافة أو إزالة)
...
exports.getUserProfile = async (req, res) => {
  try {
    const userId = req.user.id; // يأتي من التوكن بعد التحقق

    // ابحث عن المستخدم، مع إخفاء الحقل password
    const user = await User.findById(userId).select("-password");
    if (!user) {
      return res.status(404).json({ message: "لم يتم العثور على المستخدم" });
    }
...
exports.updateUserProfile = async (req, res) => {
  // الحقول النصية: name, email, phoneNumber, address
  // الصورة الشخصية إن وجدت:
  const profilePicture = req.file ? `/uploads/${req.file.filename}` : null;

  // نتحقق من فحوصات الشكل عبر Joi
  const { error } = profileUpdateSchema.validate(
    {
      name: req.body.name,
  ...
  try {
    ...
    // إن وجدنا صورة مرفوعة، نحدّث حقل profilePicture
    if (profilePicture) {
      user.profilePicture = profilePicture;
    }
...
// موافقة الأدمن على المستخدم (تحويله لشريك)
exports.approveUser = async (req, res) => {
  const { userId } = req.params;
  ...
  try {
    ...
    res.status(200).json({ message: 'تمت الموافقة على المستخدم كشريك بنجاح', user });
  } catch (error) {
    console.error('حدث خطأ أثناء الموافقة على المستخدم:', error);
    res.status(500).json({ message: 'حدث خطأ أثناء الموافقة على المستخدم' });
  }
...
// الدالة الأصلية (تُسجل الشريك فورًا) - أبقيناها دون حذف
exports.registerPartner = async (req, res) => {
  try {
    const {
      description,
    ...
    user.taxNumber = taxNumber;
    user.website = website;
    // هذه الدالة تسجله شريكًا مباشرة
    user.role = "partner";
...
// الدالة الجديدة: requestPartner (تجعل partnerStatus = "pending" ولا تغيّر الدور)
exports.requestPartner = async (req, res) => {
  try {
    ...
    // إن كان هناك طلب سابق بانتظار الموافقة
    ...
    // لا نغيّر الدور إلى partner
    user.partnerStatus = "pending"; // طلب بانتظار الموافقة
...
exports.getRentalHistory = async (req, res) => {
  try {
    const userId = req.user.id;
    // ابحث عن سجل التأجير لهذا المستخدم
    // هذه مجرد مثال: تأكد أن لديك مودل Rental أو طريقة لتخزين معلومات التأجير
    const rentals = await Rental.find({ userId: userId });
    // قد ترغب في populate("equipmentId")
